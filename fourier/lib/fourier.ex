defmodule Fourier do
  ...
  def calc([]), do: calc([@file_out, @threads])
  def calc([file_out]), do: calc([file_out, @threads])
  def calc([file_out, threads]) do
    threads = case Integer.parse(threads) do
      {d, _} -> d
      _ -> @threads
    end
    work(file_out, threads)
  end

  defp work(file_out, threads) do
    # обрабатываем входные данные
    {data, n} =  IO.stream(:stdio, :line)
                |> Enum.reduce(
                     {%{}, 0},
                     fn x, {acc, i} ->
